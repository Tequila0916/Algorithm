package org.tequila.system.class39;
...
/**
 * @ClassName Code01_SubsquenceMaxModM
...
public class Code01_SubsquenceMaxModM {
    ...
    public void process(int[] arr, int index, int sum, HashSet<Integer> set) {
        if (index == arr.length) {
            set.add(sum);
        } else {
    ...
    public void process4(int[] arr, int index, int sum, int end, int m, TreeSet<Integer> sortSet) {
        if (index == end + 1) {
            sortSet.add(sum % m);
        } else {
